...
/*
 * Columns layout + gaps
 * Master clients arranged as equal-width columns; stack clients stacked vertically on the right.
 */
static void
col(Monitor *m)
{
	...
	if (m->nmaster && n > m->nmaster) {
		sh = m->wh - 2*oh - ih * (n - m->nmaster - 1);
		sw = (mw - iv) * (1 - m->mfact);
		mw = mw - iv - sw;           /* master area total width */
		sx = mx + mw + iv;
		mw -= iv * (MIN(n, m->nmaster) - 1); /* distributable width (subtract inter-column gaps) */
	} else {
		mw -= iv * (n - 1);
	}
	...
	for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
		if (i < m->nmaster) {
			...
			resize(c, sx, sy, sw - (2*c->bw),
				sh * (c->cfact / sfacts) + ((i - m->nmaster) < srest ? 1 : 0) - (2*c->bw), 0);
			sy += HEIGHT(c) + ih;
		}
	}
}
